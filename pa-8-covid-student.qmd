...
california_covid <- cases |> 
  filter(state == "California") |> 
  mutate(date = ymd(date),
         month = month(date, label = TRUE, abbr = FALSE),
         year = year(date)) |> 
  unite(col = "month_year", c(month, year)) |>
  select(state, month_year, cases_daily)|>
  group_by(month_year)|>
...
```{r}
#| label: month-findings
# view(new_df)

california_covid |>
   ggplot(aes(x = fct_reorder(month_year, my(month_year)), 
              y = cases_daily))+
  geom_col() + 
  coord_flip()+
  labs( x = "Month and year",
        y = "Cases",
        title = "Covid numbers in California")
...
california_covid|>
  ...
  ggplot( aes( x = Month,
               y = cases_daily))+
  geom_col()+
  labs( x = "Months in the year 2021",
        y = "Cases Count",
        title = "Covid numbers in California")+
  coord_flip()
...
```{r}
#| label: "California Covid Deaths"


california_covid_deaths <- cases |> 
  filter(state == "California") |> 
  mutate(date = ymd(date),
         month = month(date, label = TRUE, abbr = FALSE),
         year = year(date)) |> 
  unite(col = "month_year", c(month, year)) |>
  select(state, month_year, deaths_daily)|>
  group_by(month_year)|>
  summarise(deaths = sum(deaths_daily))
```

```{r}
#| label: "Covid Death Graphs"
california_covid_deaths|>
  separate(month_year, c("Month", "Year"))|>
  filter( Year == 2021)|>
  ...
  labs( x = "Months in the year 2021",
        ...
        subtitle = " Which month in the year 2021 had the most covid deaths?")+
  coord_flip()
